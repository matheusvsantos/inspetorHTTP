...
void getHTTPFile(struct http_request get, int new_socket, int flag)
{
  struct hostent *hp;
...
void createSpider(struct http_request request){
  FILE *fp, *ftree;
  int i = 0;
  ...
  fp = fopen(request.complete_path, "r");
  if (fp == NULL)
...
print_hello(GtkWindow *window){
  ...
  dialog = gtk_message_dialog_new(window,
                                  ...
                                  GTK_MESSAGE_ERROR,
                                  GTK_BUTTONS_CLOSE,
                                  "Verificar arquivos baixados na pasta.");
  gtk_dialog_run(GTK_DIALOG(dialog));

}

static void
spider(GtkWindow *window){
  GtkWidget *dialog;
  char source[10000] = "";
  struct http_request httpA;
  FILE *fp1 = fopen("request.txt", "r");
  if (fp1 != NULL) {
    size_t newLen = fread(source, sizeof(char), 10000, fp1);
    if ( ferror( fp1 ) != 0 ) {
  ...
  decouple(source,&httpA);
  createSpider(httpA);
  FILE *tree = fopen("tree.txt", "r");
  if (tree != NULL) {
    size_t newLen = fread(source, sizeof(char), 10000, tree);
    if ( ferror( tree ) != 0 ) {
        fputs("Error reading file", stderr);
    } else {
        source[newLen++] = '\0'; /* Just to be safe. */
...
static void view_request(GtkWindow *window){
  GtkWidget *dialog;
...
static void view_response(GtkWindow *window){
  GtkWidget *dialog;
...
static void response(GtkWindow *window){
  GtkWidget *dialog;
  ...
  socket_id = atoi(source);

  FILE *arq;
  char buffer[BUFSIZ];
  arq = fopen("response.txt", "r");
  while(fread(&buffer, 1, BUFSIZ, arq) == BUFSIZ ){
    send(socket_id, buffer, BUFSIZ, 0);
  }
  ...
  dialog = gtk_message_dialog_new(window,
                                  ...
                                  GTK_MESSAGE_ERROR,
                                  GTK_BUTTONS_CLOSE,
                                  "Conteudo enviado para o Browser");
  gtk_dialog_run(GTK_DIALOG(dialog));

}

static void request(GtkWindow *window){
  GtkWidget *dialog;
         ...
         gpointer user_data)
{
  ...
  g_signal_connect_swapped(button, "clicked", G_CALLBACK(request), window);
  ...
  g_signal_connect_swapped(button, "clicked", G_CALLBACK(view_request), window);
  ...
  g_signal_connect_swapped(button, "clicked", G_CALLBACK(response), window);
  ...
  g_signal_connect_swapped(button, "clicked", G_CALLBACK(view_response), window);
  ...
  g_signal_connect_swapped(button, "clicked", G_CALLBACK(spider), window);
...
int main(int argc, char *argv[])
{
  ...
  if ((pid = fork()) == 0) {
    ...
    while (1)
    {
      ...
      FILE * socket1 = fopen("socket.txt", "w+");
      fprintf(socket1, "%d",new_socket);
      fclose(socket1);
      ...
      if(strcmp(httpGet.host,"g.symcd.com") != 0 && strcmp("sr.symcd.com",httpGet.host) != 0 && strcmp("ocsp.digicert.com",httpGet.host) != 0 ){
        if(cached(httpGet.complete_path)){
          ...
          getHTTPFile(httpGet, new_socket,1);
        }
      }else{
